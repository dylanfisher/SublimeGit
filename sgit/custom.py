...
class GitCustomCommand(WindowCommand, GitCmd, GitErrorHelper):
    ...
    * **syntax**: If output is set to ``"view"``, the new buffer will get this syntax file. Should be a name along the
    ...
    def run(self, cmd=None, output="view", syntax=None, run_async=False, **kwargs):
        # "async" became a reserved keyword in Python 3.7, so it can no longer be a
        # parameter name. Keep accepting it from user-defined commands/keybindings.
        run_async = kwargs.get('async', run_async)
        repo = self.get_repo()
        if not repo:
            return
